...
class TestParseStrictString:
    ...
    def test_parse_strict_string_should_work_with_ms(self):
    ...
    def test_should_return_nan_if_invalid(self):
        assert math.isnan(parse_strict("\u2603"))
        assert math.isnan(parse_strict("10-.5"))
...
class TestParseStrictLongString:
    ...
    def test_parse_strict_long_string_should_work_with_decimals(self):
...
class TestParseStrictInvalidInputs:
    def test_should_throw_an_error_when_parse_strict_invalid_input_empty_string(self):
        with pytest.raises(Exception):
            parse_strict("")

    def test_should_throw_an_error_when_parse_strict_invalid_input_length_over_100(self):
        with pytest.raises(Exception):
            parse_strict("▲" * 101)

    def test_should_throw_an_error_when_parse_strict_invalid_input_undefined(self):
        with pytest.raises(Exception):
            parse_strict(None)

    def test_should_throw_an_error_when_parse_strict_invalid_input_null(self):
        with pytest.raises(Exception):
            parse_strict(None)

    def test_should_throw_an_error_when_parse_strict_invalid_input_array(self):
        with pytest.raises(Exception):
            parse_strict([])

    def test_should_throw_an_error_when_parse_strict_invalid_input_object(self):
        with pytest.raises(Exception):
            parse_strict({})

    def test_should_throw_an_error_when_parse_strict_invalid_input_nan(self):
        with pytest.raises(Exception):
            parse_strict(float("nan"))

    def test_should_throw_an_error_when_parse_strict_invalid_input_infinity(self):
        with pytest.raises(Exception):
            parse_strict(float("inf"))

    def test_should_throw_an_error_when_parse_strict_invalid_input_negative_infinity(self):
        with pytest.raises(Exception):
            parse_strict(float("-inf"))
